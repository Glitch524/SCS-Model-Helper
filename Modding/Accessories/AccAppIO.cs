...
class AccAppIO {
	...
	public static void LoadAccAddon(Window window, AccAddonBinding binding, string dedFile) {
		...
		static void ReadLists(ObservableCollection<string> list, string reads) {
			...
			if (reads.Length == 0)
				return;
			foreach (var i in reads.Split(TruckDefault.ItemSplit)) {
				list.Add(i);
			}
		}
	...
	private static void SetSelected(int[] indexes, List<string> lines, bool isETS2, AccAddonBinding binding) {
		...
		foreach (var truck in truckList) {
			...
			while (i < lines.Count) {
				string? line = lines[i];
				var s = line.Split(TruckDefault.ItemSplit);
			...
			if (line != null && line.StartsWith($"{FileHeader}")) {
				...
				if (trucksHeader != null && trucksStrings.Count > 0) {
					...
					foreach (var tString in trucksStrings) {
						var s = tString.Split(TruckDefault.ItemSplit);
	...
	public static List<Truck> LoadDefaultTrucks(bool ets2) {
		if (ets2)
			return TruckDefault.DefaultTrucksETS2;
		else
			return TruckDefault.DefaultTrucksATS;
	}
}
